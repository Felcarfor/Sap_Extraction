...
from config import service_account_info, local_path, folder_id
...
def delete_existing_files(file_name, service): #deleta os arquivos no drive
    try:
...
def Upload_File(file_name, service):#Sobe os arquivos para a pasta do Drive
    try:
        mime_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        full_path = local_path + file_name
        delete_existing_files(file_name, service)
...
def download_and_load_xlsx(file_id, creds):
    try:
        drive_service = build('drive', 'v3', credentials=creds)
    ...
    try:
        ...
        for filtered_file in filtered_files:
            try:
                ...
                df = download_and_load_xlsx(filtered_file['id'], creds)
...
def send_log_to_sheets(log_message: str, message_type: str, system: str, creds, spreadsheet_id: str,env) -> None:
    """
    ...
    values = [[now,system, message_type, log_message,env ]]
    body = {'values': values}
